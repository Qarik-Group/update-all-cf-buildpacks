...
sed -i "s%.*\b$BUILDPACK_NAME\b.*$%cf update-buildpack -s cflinuxfs3 --enable $BUILDPACK_NAME -p https://github.com/$GITHUB_ORG/$GITHUB_REPO/releases/download/v${version}/${filename}%" update-only.sh
